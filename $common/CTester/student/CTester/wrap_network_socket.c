...
ssize_t __wrap_recv(int sockfd, void *buf, size_t len, int flags)
{
    ...
    if (FAIL(failures.recv)) {
        ...
        if (failures.recv_fun) {
            return (stats.recv.last_return = ((ssize_t(*)(int, void *, size_t, int))failures.recv_fun)(sockfd, buf, len, flags));
        } else {
...
ssize_t __wrap_recvfrom(int sockfd, void *buf, size_t len, int flags, struct sockaddr *src_addr, socklen_t *addrlen)
{
    ...
    if (FAIL(failures.recvfrom)) {
        ...
        if (failures.recvfrom_fun) {
            return (stats.recvfrom.last_return = ((ssize_t(*)(int, void *, size_t, int, struct sockaddr *, socklen_t *))failures.recvfrom_fun)(sockfd, buf, len, flags, src_addr, addrlen));
        } else {
            errno = failures.recvfrom_errno;
            return (stats.recvfrom.last_return = failures.recvfrom_ret);
        }
    }
    failures.recvfrom = NEXT(failures.recvfrom);
    ssize_t ret = -1;
...
ssize_t __wrap_send(int sockfd, const void *buf, size_t len, int flags)
{
    ...
    if (stats.send.last_params_buffered.buf && stats.send.last_params.len >= len) {
        memcpy((void *)stats.send.last_params_buffered.buf, buf, len);
    }
...
ssize_t __wrap_sendto(int sockfd, const void *buf, size_t len, int flags, const struct sockaddr *dest_addr, socklen_t addrlen)
{
    ...
    if (stats.sendto.last_params_buffered.buf && stats.sendto.last_params.len >= len) {
        memcpy((void *)stats.sendto.last_params_buffered.buf, buf, len);
    }
    if (stats.sendto.last_params_buffered.dest_addr && stats.sendto.last_params.addrlen >= addrlen) {
        memcpy((void *)stats.sendto.last_params_buffered.dest_addr, dest_addr, addrlen);
    }
    /*if (dest_addr != NULL) {
