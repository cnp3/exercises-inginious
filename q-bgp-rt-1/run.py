...
tag_question = ["q1", "q2", "q3", "q4"] # ID of each question
correct = {"q1": ["ASa"], "q2": ["ASb:ASa", "ASa", "ASd:ASb:ASa"], "q3": ["ASb:ASa", "ASe:ASc:ASb:ASa"],"q4": ["ASc:ASb:ASa"]} # answer to each question (with AS used in mapping)
AS_ref = json[index]["ASa"]
# feedback for each question
feedback_list = {"q1": [[f"Your best path does not end to {AS_ref}", f"Your best path is incorrect"]], "q2": [[f"Your best path does not end to {AS_ref}", f"Your best path is incorrect"],[f"Your second path does not end to {AS_ref}", f"Your second path is incorrect"],[f"Your third path does not end to {AS_ref}", f"Your third path is incorrect"]], "q3": [[f"Your best path does not end to {AS_ref}", f"Your best path is incorrect"],[f"Your second path does not end to {AS_ref}", f"Your second path is incorrect"]],"q4": [[f"Your best path does not end to {AS_ref}", f"Your best path is incorrect"]]} 
