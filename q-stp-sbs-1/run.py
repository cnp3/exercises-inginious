...
tag_list = ["q1", "q2", "q3", "q4", "q5", "q6", "q7", "q8", "q9", "q10"]


grade += checkQRoot("Sa", 10, "q1", mapping)
if(grade == 10):
    grade += checkQBPDU({"R":"Sa", "C":0, "T":"Sa"}, 10, "q2", mapping)
    grade += checkQBPDU({"R":"Sa", "C":1, "T":"Sb"}, 10, "q3", mapping)
    grade += checkQBPDU({"R":"Sa", "C":1, "T":"Sc"}, 10, "q4", mapping)
else:
...
if(grade == 40):
    grade += checkQBestBPDU("Sa", 10, "q5" , mapping)
    grade += checkQBestBPDU("Sa", 10, "q6" , mapping)
    grade += checkQBestBPDU("Sb", 10, "q7" , mapping)
elif(grade >= 10):
    for tag in tag_list[4:]:
        feedback.set_problem_result("failed", tag)
        feedback.set_problem_feedback("This question will be corrected when you have answered questions 2, 3 and 4 correctly.", tag)

if(grade == 70):
    grade += checkQPortState({"1":"designated", "2":"designated"}, 10, "q8", ["1", "2"])
    grade += checkQPortState({"1":"root", "2":"designated"}, 10, "q9", ["1", "2"])
    grade += checkQPortState({"1":"blocked", "2":"root"}, 10, "q10", ["1", "2"])
elif(grade >= 40):
    for tag in tag_list[7:]:
        feedback.set_problem_result("failed", tag)
        feedback.set_problem_feedback("This question will be corrected when you have answered questions 5, 6 and 7 correctly.", tag)
