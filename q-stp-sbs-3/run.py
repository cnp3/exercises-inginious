...
tag_list = ["q1", "q2", "q3", "q4", "q5", "q6", "q7", "q8", "q9", "q10", "q11", "q12", "q13", "q14"]


grade += checkQRoot("Sa", 10, "q1", mapping)
if(grade == 10):
    grade += checkQBPDU({"R":"Sa", "C":2, "T":"Sb"}, 10, "q2", mapping)
    grade += checkQBPDU({"R":"Sa", "C":1, "T":"Sc"}, 10, "q3", mapping)
    grade += checkQBPDU({"R":"Sa", "C":1, "T":"Sd"}, 10, "q4", mapping)
    grade += checkQBPDU({"R":"Sa", "C":0, "T":"Sa"}, 10, "q5", mapping)
else:
...
if(grade == 50):
    grade += checkQBestBPDU("Sc", 10, "q6" , mapping)
    grade += checkQBestBPDU("Sa", 10, "q7" , mapping)
    grade += checkQBestBPDU("Sd", 10, "q8" , mapping)
    grade += checkQBestBPDU("Sa", 10, "q9" , mapping)
    grade += checkQBestBPDU("Sc", 10, "q10" , mapping)
elif(grade >= 10):
...
if(grade == 100):
    grade += checkQPortState({"1":"root", "2":"blocked"}, 10, "q11", ["1", "2"])
    grade += checkQPortState({"1":"designated", "2":"root"}, 10, "q12", ["1", "2"])
    grade += checkQPortState({"1":"designated", "2":"root"}, 10, "q13", ["1", "2"])
    grade += checkQPortState({"1":"designated", "2":"designated"}, 10, "q14", ["1", "2"])
elif(grade >= 50):
    for tag in tag_list[10:]:
        feedback.set_problem_result("failed", tag)
        feedback.set_problem_feedback("This question will be corrected when you have answered questions 6, 7, 8, 9 and 10 correctly.", tag)

# set grade and global result
feedback.set_grade(grade/(1.4))
if int(grade) == 140:
